...
class MDLP_Discretizer(TransformerMixin):
    ...
    def transform(self, X, inplace=False):
        ...
        ################ ADDITION WITH RESPECT TO THE ORIGINAL CODE ##########################
        discretized = np.vstack([self.threshold_encoding(x, self._cuts) for x in discretized])
        ######################################################################################
        return discretized
    ...
    # function that converts the one hot encoding into the one based on thresholds #
    def threshold_encoding(self, sample, thresholds):
        res = []
        for idx, value in enumerate(sample):
            aux = [1 if i >= value else 0 for i in np.arange(len(thresholds[idx]))]
            res.append(np.array(aux))
        res = np.hstack(res).astype(float)
        return res
    ################################################################################