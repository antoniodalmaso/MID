...
from discretizers.MDLP import MDLP_Discretizer
...
X_train_mid_entropy = discr_mid_entropy.fit_transform(X_train, y_train, n_features=X_train_mdlp.shape[1])
X_test_mid_entropy = discr_mid_entropy.transform(X_test, n_features=X_train_mdlp.shape[1])

discr_mid_gini = MinimumImpurityDiscretizer(metric = "gini")
X_train_mid_gini = discr_mid_gini.fit_transform(X_train, y_train, n_features=X_train_mdlp.shape[1])
X_test_mid_gini = discr_mid_gini.transform(X_test, n_features=X_train_mdlp.shape[1])


# training the trees
clf_mdlp = DL85Classifier(
    max_depth = max_depth,
    min_sup = 5,
    time_limit = 300 # 5 minuti
)
clf_mdlp.fit(X_train_mdlp, y_train)
...
# testing the trees
y_pred_mdlp = clf_mdlp.predict(X_test_mdlp)
...
# saving the results
results_mdlp = (
    X_train_mdlp.shape[1],                 # n. features
    clf_mdlp.accuracy_,                    # train accuracy
    accuracy_score(y_test, y_pred_mdlp),   # test accuracy
    clf_mdlp.runtime_,                     # runtime
    clf_mdlp.timeout_                      # timeout reached (boolean)
)
results_mid_entropy = (
...
results = (results_mdlp, results_mid_entropy, results_mid_gini)

if not os.path.exists(f'partial_exp_data/MDLP/{ds_name}/{max_depth}_5'):
    os.makedirs(f'partial_exp_data/MDLP/{ds_name}/{max_depth}_5')

file_name = f"partial_exp_data/MDLP/{ds_name}/{max_depth}_5/fold_{split_idx}"
